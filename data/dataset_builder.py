    ...
    for i in range(len(l)):
        ...
        if len(line[0]) > 1:
            for j in range(len(line[0]) - 1):
                draw.line((line[0][j], line[1][j], line[0][j+1], line[1][j+1]), fill=(0, 0, 0), width=3)
        else:
...
def json_to_img(json_folder: str,
    ...
    @param labels {Optional[List[str]]} - The labels to consider, None for all labels.
    @param delete_on_error {bool} - Delete the output folder if an error occurs.
    @param test_size {float} - The proportion of the dataset to include in the test split.
    @param val_size {float} - The proportion of the dataset to include in the validation split.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("output_path", type=str, help="The folder where the images will be saved")
    parser.add_argument("--max_images", type=int, default=250, help="The number of images to save per json file")
    parser.add_argument("--labels", type=str, nargs="+", help="The labels to consider")
    parser.add_argument("--delete_on_error", action="store_true", help="Delete the output folder if an error occurs")
    ...
    json_to_img(args.input_path, args.output_path, args.max_images, args.labels, args.delete_on_error, args.test_size, args.val_size)